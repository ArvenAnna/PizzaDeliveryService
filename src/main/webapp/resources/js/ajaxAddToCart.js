...
$(".addPizza").on('click', {url:"addpizza"}, addPizzaButton);
...
//                            $("#" + i).append('<div class="" style="text-align: center;"><a href=""><span id="" class="cart" style="">xxx</span><br></a></div><img src="" alt="Pizza image" style="margin-left: 40px"/><div style="text-align: center;">Цена: <span class="cart" style="">0</span> грн.</br></div><div style="margin-left: 100px;"><button class="glyphicon glyphicon-plus"></button><span class=""><b>1</b></span> шт.<button class="glyphicon glyphicon-minus"></button></div>');
...
function addPizzaButton (event) {
        ...
        $.ajax({
            ...
            success: function (data) {
                alert(JSON.stringify(data, " ", 4));
                if (data["exception"]) {
                    $(".errorMessage").html("<span style='color: red; font-weight: bold'>В Вашей корзине может быть не более 10 пицц</span>");
                ...
                $(".glyphicon-plus").on('click', {url:"addpizza"}, addPizzaButton);
                $(".glyphicon-minus").on('click', {url:"delpizza"}, addPizzaButton);
            },
            error: function (xhr, ajaxOptions, thrownError) {
                alert(xhr.status);
                alert(thrownError);
